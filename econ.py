...
class World:
	...
	def update_econ(self):
		...
		self.global_needs = self.get_needs()
		for individual in factory.get(Individual):
...
class Entity:
	...
	def purchase(self, item, qty, acct_buy='Inventory', acct_sell='Inventory', buffer=False):
		...
		if global_qty >= qty or item_type == 'Service':
			...
			if item_type == 'Service':
				...
				#else:
				outcome = counterparty.produce(item, qty, buffer=buffer)
				#print('Outcome: \n{}'.format(outcome))
				if not outcome:
					return
				try:
		...
		for requirement in requirements_details:
			...
			if req_item_type == 'Time':
				...
				if building < (req_qty * qty): # TODO FIx qty required
					...
					if not entries:
						entries = self.produce(req_item, req_qty * qty, buffer=True)
					if not entries:
						entries = []
						incomplete = True
						#return
				...
				if ((equip_qty * capacity) / (qty * req_qty)) < 1: # TODO Test item with capacity
					...
					# TODO Undecided if below produce func should be in final
					if not entries:
						entries = self.produce(req_item, required_qty, buffer=True)
					if not entries:
						entries = []
						incomplete = True
						#return
	...
	def produce(self, item, qty, debit_acct='Inventory', credit_acct='Goods Produced', debit_acct_wip='WIP Inventory', credit_acct_wip='Goods Produced', desc=None ,price=None, buffer=False):
		...
		if time_required and item_type != 'Service':
			produce_entry = [ ledger.get_event(), self.entity_id, world.now, desc, item, price, qty, debit_acct_wip, credit_acct_wip, price * qty ]
	...
	def inheritance(self):
		...
		while True:
			...
			if individual.entity_id == self.entity_id:
				...
				break
		print('Inheritance bequeathed to {} from {}'.format(counterparty.name, self.name))
		ledger.set_entity(self.entity_id)
