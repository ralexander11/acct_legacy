...
class Entity:
	...
	def purchase(self, item, qty, acct_buy='Inventory', acct_sell='Inventory', acct_rev='Sales', wip_acct='WIP Inventory', buffer=False):
		...
		if item_type == 'Service':
			...
			if not serv_prices:
				...
				return
			counterparty_id = max(serv_prices, key=lambda k: serv_prices[k])
		...
		for requirement in requirements_details:
			...
			if req_item_type == 'Time':
				...
				if ((equip_qty * capacity) / (qty * req_qty)) < 1: # TODO Test item with capacity
					...
					# Attempt to purchase before producing self if makes sense
					result = self.purchase(req_item, required_qty, acct_buy='Equipment', wip_acct='WIP Equipment')#, buffer=True)
					# TODO Undecided if below produce func should be in final
					if not result:
						#entries = []
	...
	def check_demand(self, v=False):
		...
		for item in self.produces:
			...
			self.adj_price(item, qty_existance, direction='up')
	...
	def accru_wages(self, job, counterparty, labour_hours, wage=None, buffer=False, check=False):
		...
		if recently_paid and not incomplete:
			if counterparty.hours > 0:
				...
				if job != 'Study':
					counterparty.adj_price(job, labour_hours, direction='up')
			else:
				if not incomplete:
	...
	def order_subscription(self, item, counterparty, price, qty=1, buffer=False):
		...
		if cash >= price:
			...
			self.adj_price(item, qty=1, direction='up')
