...
class World:
	def __init__(self, factory, population=2):
		...
		#self.items = self.items.assign(price=INIT_PRICE) # Initial price for all items
		# self.items = self.items.assign(entity_id=0)
		# print('Items Config: \n{}'.format(self.items))
		self.setup_prices()
	...
	def setup_prices(self):
		...
		self.prices = pd.DataFrame(data={'item_id': self.items.index,'entity_id': 0,'price': INIT_PRICE})
	...
	def create_land(self, item, qty):
		price = self.get_price(item, 0)
		land_entry = [ ledger.get_event(), 0, self.now, item + ' created', item, price, qty, 'Land', 'Natural Wealth', price * qty ]
	...
	def update_econ(self):
		...
		if str(self.now) == '1986-10-01':
			...
			for individual in factory.get(Individual):
				individual.capitalize(amount=24000)#12000 # Hardcoded for now
		...
		print()
		# print(ledger.get_qty(['Rock','Wood','Paper','Food','Wood Chips'], ['Inventory'], show_zeros=True, by_entity=True)) # Temp for testing
		# print()
		...
		print()
		self.entities = accts.get_entities().reset_index()
		self.inventory = ledger.get_qty(items=None, accounts=['Inventory','Equipment','Buildings'], show_zeros=False, by_entity=True)
...
class Entity:
	...
	def adj_price(self, item, qty=1, rate=1.01):
		if item == self.name:
		...
		if cash >= (qty * price):
			...
			if (qty <= qty_avail or item_type == 'Service'):
				if item_type is not None and item_type != 'Service':
					...
					avg_price = cost / qty
					cogs_entry = [ ledger.get_event(), counterparty.entity_id, world.now, desc_sell, item, avg_price, qty, 'Cost of Goods Sold', acct_sell, cost ]
					...
					counterparty.adj_price(item, qty, rate=1.01)
					if buffer:
						return purchase_event, cost
					ledger.journal_entry(purchase_event)
					return purchase_event, cost #True, cost
				else:
		...
		if item_type != 'Service':
			...
			while qty > 0:
				...
				if counterparty.entity_id == self.entity_id:
					print('{}-{} unable to transact with themselves.'.format(self.name, self.entity_id))
					return
				if purchase_qty > qty:
					purchase_qty = qty
				#print('Purchase QTY: {}'.format(purchase_qty))
				result_tmp, cost = self.transact(item, price=world.get_price(item, counterparty.entity_id), qty=purchase_qty, counterparty=counterparty, acct_buy=acct_buy, acct_sell=acct_sell, item_type=item_type, buffer=buffer)
				if not result_tmp:
					break
				result += result_tmp
			...
			if qty_wanted > purchased_qty and item_type != 'Land':
				...
				if qty_wanted > 0:
					if cost:
						self.item_demanded(item, qty_wanted - purchased_qty, cost) # TODO Is this needed?
					else:
	...
	def consume(self, item, qty, need=None, buffer=False):
		...
		if qty_held >= qty:# or buffer:
			...
			# else: # TODO Remove
			ledger.set_entity(self.entity_id)
			print('{}-{} getting historical cost of {} for {} qty.'.format(self.name, self.entity_id, item, qty))
			cost = ledger.hist_cost(qty, item, 'Inventory')#, v=True)
			ledger.reset()
			#print('Cost: {}'.format(cost))
			price = cost / qty
		...
		for requirement in requirements_details:
			...
			if req_item_type == 'Time':
				...
				if land < (req_qty * qty):
					...
					result = self.purchase(req_item, req_qty * qty, 'Land')#, buffer=True)
					if not result:
						...
						result = self.claim_land(qty_claim, price=world.get_price(req_item, 0), item=req_item)#, buffer=True)
						#print('Land Entries: \n{}'.format(entries))
						if not result:
							#entries = []
							incomplete = True
				...
				if time_required: # TODO Handle land in use during one tick
					entries = self.in_use(req_item, req_qty * qty, world.get_price(req_item, self.entity_id), 'Land', buffer=True)
				...
				print('{}-{} requires {} {} building (Capacity: {}) to produce {} and has {}.'.format(self.name, self.entity_id, req_qty, req_item, capacity, item, building))
				if ((building * capacity) / (qty * req_qty)) < 1:
					...
					result = self.purchase(req_item, required_qty, acct_buy='Buildings', wip_acct='Building Under Construction')#, buffer=True)
					# TODO Undecided if below produce func should be in final
					if not result:
						#entries = []
						result, req_time_required = self.produce(req_item, required_qty)#, buffer=True)
						if not result:
							#entries = []
							incomplete = True
							#return
				...
				if capacity is None:
					capacity = 1
				capacity = float(capacity)
				print('{}-{} requires {} {} equipment (Capacity: {}) to produce {} and has {}.'.format(self.name, self.entity_id, req_qty, req_item, capacity, item, equip_qty))
				if ((equip_qty * capacity) / (qty * req_qty)) < 1: # TODO Test item with capacity
					...
					# Attempt to purchase before producing self if makes sense
					result = self.purchase(req_item, required_qty, 'Equipment', wip_acct='WIP Equipment')#, buffer=True)
					# TODO Undecided if below produce func should be in final
					if not result:
						#entries = []
				...
				if modifier:
					...
					modifier = 0
				material_qty_held = ledger.get_qty(items=req_item, accounts=['Inventory'])
				...
				print('{}-{} will attempt to purchase the {} service.'.format(self.name, self.entity_id, req_item))
				...
				if item_type == 'Job' or item_type == 'Labour':
					...
					if ((workers * capacity) / (qty * req_qty)) < 1:
						...
						for _ in range(required_qty):
							entries = self.hire_worker(job=req_item, qty=1, buffer=True)
							if not entries:
								entries = []
								incomplete = True
								#return
				...
				if item_type == 'Job' or item_type == 'Labour':
					if type(self) == Individual:
						...
						print('{}-{} has {} education hours for {} and requires {}'.format(self.name, self.entity_id, edu_status, req_item, req_qty))
	...
	def check_inv(self, v=False):
		...
		for item in self.produces:
			...
			if qty_inv > 0:
				self.adj_price(item, qty_inv, rate=0.99)
		ledger.reset()
	...
	def incorporate(self, item=None, price=None, qty=None, name=None):
		...
		#print('Cash: {}'.format(cash))
		if price * qty > cash:
			print('{}-{} does not have enough cash to incorporate {}.'.format(self.name, self.entity_id, name))
			return
				...
				if item_type == 'Education':
					if demand_row['entity_id'] == self.entity_id: # TODO Could filter df for entity_id first
						if demand_row['item_id'] == item:
							...
							if demand_row['reason'] != 'cost':
								qty_existance = demand_row['qty']
							break
				else:
					if demand_row['item_id'] == item:
						qty += demand_row['qty']
	...
	def pay_wages(self, jobs=None, counterparty=None):
		...
		for job in jobs:
			...
			if not wages_payable:
				...
				# Get counterparty
				rvsl_txns = ledger.gl[ledger.gl['description'].str.contains('RVSL')]['event_id'] # Get list of reversals
				wages_paid_txns = ledger.gl[( (ledger.gl['debit_acct'] == 'Wages Payable') & (ledger.gl['credit_acct'] == 'Cash') & (ledger.gl['item_id'] == job) ) & (~ledger.gl['event_id'].isin(rvsl_txns))]['event_id']
				#print('Wages Paid TXN Event IDs: \n{}'.format(wages_paid_txns))
				wages_pay_txns = ledger.gl[( (ledger.gl['debit_acct'] == 'Wages Expense') & (ledger.gl['credit_acct'] == 'Wages Payable') ) & (~ledger.gl['event_id'].isin(wages_paid_txns)) & (~ledger.gl['event_id'].isin(rvsl_txns))]
				job_wages_pay_txns = wages_pay_txns.loc[wages_pay_txns['item_id'] == job]
				#print('Wages Pay TXNs: \n{}'.format(job_wages_pay_txns))
				counterparties = job_wages_pay_txns.shape[0]
				#print('Number of Counterparties: {}'.format(counterparties))
				pay_wages_event = []
				for n in range(counterparties):
					counterparty, event_id = self.get_counterparty(wages_pay_txns, rvsl_txns, job, 'Wages Receivable', n=n, allowed=Individual)
				...
				counterparty.adj_price(job, labour_hours, rate=0.99)
				return
	...
	def accru_wages(self, job, counterparty, labour_hours, wage=None, buffer=False, check=False):
		...
		if recently_paid and not incomplete:
			if counterparty.hours > 0:
				...
				if job != 'Study':
					counterparty.adj_price(job, labour_hours, rate=1.01)
			else:
				if not incomplete:
	...
	def hire_worker(self, job, counterparty=None, price=0, qty=1, buffer=False):
		...
		if first_pay:
			self.adj_price(job, qty=1, rate=1.01)
		if buffer:
	...
	def fire_worker(self, job, counterparty, price=0, qty=-1, quit=False, buffer=False):
		...
		counterparty.adj_price(job, qty=1, rate=0.99)
		if buffer:
	...
	def pay_salary(self, job, counterparty, salary=None, labour_hours=None, buffer=False, first=False, check=False):
		...
		if cash >= (salary * labour_hours) and not incomplete:
			...
			counterparty.adj_price(job, labour_hours, rate=1.002)
			if buffer:
		...
		for corporation in factory.get(Corporation):
			...
			for item in corporation.produces:
				if item == subscription:
					return corporation
		print('No company exists that can provide the {} subscription for {}-{}.'.format(subscription, self.name, self.entity_id))
	...
	def order_subscription(self, item, counterparty, price, qty=1, buffer=False):
		...
		if cash >= price:
			...
			self.adj_price(item, qty=1, rate=0.99)
	...
	def cancel_subscription(self, item, counterparty, price=0, qty=-1):
		...
		counterparty.adj_price(item, qty=1, rate=0.99)
	...
	def pay_subscription(self, item, counterparty, price=None, qty='', buffer=False, first=False, check=False):
		...
		for _ in range(subscription_state):
			...
			if cash >= price and not incomplete:
				...
				counterparty.adj_price(item, qty=1, rate=1.002)
				if buffer:
					continue
...
class Individual(Entity):
	...
	def birth(self, counterparty=None, amount=None):
		...
		ledger.reset()
		if cash1 >= amount / 2:
			gift_entry1 = [ ledger.get_event(), self.entity_id, world.now, 'Gift cash to child', '', '', '', 'Wealth', 'Cash', amount / 2 ]
		else:
			print('{} does not have ${} in cash to give birth with {}.'.format(self.name, amount / 2, counterparty.name))
			return
		ledger.set_entity(counterparty.entity_id)
		cash2 = ledger.balance_sheet(['Cash'])
		ledger.reset()
		if cash2 >= amount / 2:
			gift_entry2 = [ ledger.get_event(), counterparty.entity_id, world.now, 'Gift cash to child', '', '', '', 'Wealth', 'Cash', amount / 2 ]
		else:
			print('{} does not have ${} in cash to have child with {}.'.format(counterparty.name, amount / 2, self.name))
			return
		...
		gift_event += individual.capitalize(amount=amount, buffer=True) # TODO Hardcoded amount for now
	...
	def inheritance(self, counterparty=None):
		...
		for index, job in current_jobs.iterrows():
			...
			if worker_state >= 1: # Should never be greater than 1 for the same counterparty
				...
				ledger.reset()
				counterparty, event_id = self.get_counterparty(hire_txns, rvsl_txns, item, 'Worker Info')
		...
		for index, sub in current_subs.iterrows():
			...
			if sub_state >= 1:
				...
				ledger.reset()
				counterparty, event_id = self.get_counterparty(sub_txns, rvsl_txns, item, 'Sell Subscription')
...
class Corporation(Organization):
	def __init__(self, name, items):
		...
		self.entity_id = accts.add_entity(entity_data)
		# self.name = entity_data[0][0] + '-' + str(self.entity_id) # Experiment
		self.name = entity_data[0][0]
		self.produces = entity_data[0][13]
		if isinstance(self.produces, str):
			self.produces = [x.strip() for x in self.produces.split(',')]
